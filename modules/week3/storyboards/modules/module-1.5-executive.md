# MODULE 1.5: Executive Insights - Andrew Sneyd, FanDuel CMO
**Version:** 1.0.0 | **Last Updated:** 2025-12-05

**Purpose:** Deliver expert practitioner perspective on sports betting marketing and brand building (supports WLO 3.1, 3.2, 3.3)

**Uplimit Structure:** Inserted after Module 1 (Welcome), before Module 2 (Sponsorship Framework)

| Order | Element | Content/Purpose | Source | Implementation Notes |
|-------|---------|----------------|--------|---------------------|
| 1 | **▬ Text** ⬤ Required | Connecting intro from Module 1 | Type directly | Sets context for executive perspective |
| 2 | **ⓘ Infobox (Callout)** ⬤ Required | Context about Andrew Sneyd + viewing guide | Type directly | Blue variant, sets viewing expectations |
| 3 | **▶ Video** ⬤ Required | Andrew Sneyd Interview (~45 min) | Upload MP4 + transcript | FanDuel CMO on betting marketing, brand building |
| 3A | **📝 Practice Quiz** ⬤ Required | Andrew Sneyd Interview Check (5 questions) | Configure in Uplift | Ungraded, tests interview insights |
| 4 | **▤ Details** ○ Optional | Video transcript & executive bio | Type directly | Accordion for text version |
| 5 | **◈ AI Chat Widget** ○ Optional | "Ask questions about betting marketing" | Configure in Uplimit | Named: "Betting Marketing Q&A" |

---

## Element 1: Setting Up Andrew Sneyd's Perspective

**Copy this markdown directly into Uplimit:**

```markdown
You now understand where you're headed this week—the four learning outcomes that will transform how you analyze sponsorship ROI and sports betting's role as a growth engine. Before diving into the frameworks, you need context from someone who's actually done this work at the highest level.

**Connecting to Weeks 1-2:** You've heard from Kevin Abrams (NY Giants) about revenue ecosystems, and from Al Dak (Rogers) about media rights strategy. Now you'll hear from the marketing leader who helped build the largest sports betting brand in America.

In this module, you'll hear from **Andrew Sneyd, Chief Marketing Officer at FanDuel**. Andrew led FanDuel's explosive growth from scrappy startup to the #1 sports betting operator in the US, overseeing a marketing budget exceeding $1 billion. His insights on brand building, customer acquisition, and "Trust, Easy, Fun" will ground the sponsorship and betting frameworks you're about to explore.

Listen carefully for: How do you build a brand in a heavily regulated, competitive industry? What metrics actually matter for customer acquisition? And why did FanDuel bet everything on the "Kick of Destiny" campaign?
```

---

## Element 2: Infobox Content

**Copy this markdown directly into Uplimit:**

```markdown
Title: ◉ Executive Session with Andrew Sneyd, CMO, FanDuel

Andrew Sneyd leads marketing at FanDuel, the largest sports betting operator in North America with ~45% market share. He's overseen $1+ billion in annual marketing investment, navigated the post-PASPA legalization wave, and built a brand that grew awareness from 40% to 50% in one Super Bowl campaign.

**Viewing Guide:**
• Watch for: The "Trust, Easy, Fun" brand pillars and how they guide every decision
• Note: "The risk was in no one paying attention" - why FanDuel took calculated risks on unconventional campaigns
• Consider: "Marketing is equal parts science and art... the art is still 33%" - balancing creativity with data
• Observe: How FanDuel measures ROI across 1 billion emails per year and $1 billion in media spend

**Key Topics Covered:**
- Brand building: How FanDuel became synonymous with sports betting
- Kick of Destiny campaign: 13 million sign-ups, 40% → 50% brand awareness in one weekend
- Customer acquisition economics: What works and what doesn't at scale
- Career advice: "Be curious—those little earworms make such a difference"

This ~45-minute interview provides deep insights on building a category-defining brand in a rapidly evolving industry.

Supports WLO 3.1, 3.2, and 3.3.

**Total viewing time:** ~45 minutes
```

---

## Element 3: Video - Andrew Sneyd (FanDuel) Interview

**File:** `Interviews/Andrew, FanDuel CMO.mp4`
**Duration:** ~45 minutes
**Transcript:** `Interviews/Andrew, FanDuel CMO_Captions_English (United States).txt`
**Poster Image:** `week3-andrew-sneyd-poster.jpg` (thumbnail - to be created)
**Video Title:** "Andrew Sneyd: Building FanDuel into America's Betting Brand"
**Video Caption:** "FanDuel CMO reveals how $1B marketing investment, 'Trust Easy Fun' positioning, and the Kick of Destiny campaign built market dominance"

### Interview Content - Key Topics Covered:

**Brand Philosophy:**
- "Trust, Easy, Fun" - the three pillars guiding all marketing decisions
- Building brand in a category with regulatory constraints
- Differentiating from DraftKings and casino operators

**Kick of Destiny Campaign:**
- Super Bowl campaign that changed the brand trajectory
- "The risk was in no one paying attention"
- 13 million sign-ups around Super Bowl
- Brand awareness jumped 40% to 50%

**Customer Acquisition at Scale:**
- 1 billion emails sent per year
- $1 billion spent on TV/media - all paying back within 18 months
- 10% of all Ohio adults signed up for FanDuel in first 30 days of legalization
- 80% of sports bets are player props (not game outcomes)

**Marketing Philosophy:**
- "Marketing is equal parts science and art... the art is still 33%"
- "You can only measure what's happened in the past"
- Importance of creative differentiation in a data-driven industry

**Career Advice:**
- "Be curious—those little earworms make such a difference"
- Building a career in sports marketing

---

### Element 3A: Practice Quiz - Andrew Sneyd Interview Check

**Uplift Implementation:**
1. Select **Exercise - Practice Quiz** element
2. Configure as ungraded self-assessment
3. Add the following 5 multiple choice questions:

---

### Question 1: Brand Pillars

What are FanDuel's three brand pillars that guide all marketing decisions?

A) Speed, Excitement, Winning
B) Trust, Easy, Fun ✅
C) Data, Growth, Innovation
D) Safe, Simple, Social

**Feedback (Correct):** Correct! Andrew emphasizes "Trust, Easy, Fun" as the foundation for all FanDuel marketing decisions. "Trust" addresses integrity concerns (your money is safe, games are fair), "Easy" reduces friction in a complex category, and "Fun" connects betting to entertainment rather than gambling.

**Feedback (Incorrect):** Andrew explicitly states FanDuel's three pillars early in the interview. These guide every campaign and creative decision—listen for how he describes the brand's core promise to customers.

---

### Question 2: Kick of Destiny Campaign

According to Andrew, what was the REAL risk FanDuel faced with the Kick of Destiny Super Bowl campaign?

A) The kicker might miss the field goal on live TV
B) The campaign might offend non-bettors watching the Super Bowl
C) No one would pay attention—the campaign would go unnoticed ✅
D) Competitors would copy the concept before launch

**Feedback (Correct):** Correct! Andrew says "the risk was in no one paying attention." In a cluttered Super Bowl advertising environment, being ignored is worse than being controversial. The Kick of Destiny cut through because it was unprecedented—a live field goal attempt with real stakes during the biggest TV event of the year.

**Feedback (Incorrect):** Andrew reframes the typical risk assessment. Most marketers worry about campaigns being too bold—Andrew worried about the opposite. What did he say was the real threat in a crowded advertising environment?

---

### Question 3: Marketing Budget Scale

Approximately how much does FanDuel spend annually on TV and media advertising?

A) $100 million
B) $500 million
C) $1 billion+ ✅
D) $2.5 billion

**Feedback (Correct):** Correct! Andrew mentions FanDuel spends over $1 billion on TV and media annually—and critically, all of it pays back within 18 months. This massive budget makes FanDuel one of the largest advertisers in sports, rivaling traditional giants like Anheuser-Busch.

**Feedback (Incorrect):** Andrew provides specific scale numbers about FanDuel's marketing investment. The figure is larger than most traditional sports sponsors—think about what budget would make FanDuel one of the top 10 advertisers in all of sports.

---

### Question 4: Customer Behavior Insight

What percentage of sports bets are player prop bets (individual player performance) versus game outcome bets?

A) 20% props, 80% game outcomes
B) 50% props, 50% game outcomes
C) 80% props, 20% game outcomes ✅
D) 95% props, 5% game outcomes

**Feedback (Correct):** Correct! Andrew reveals that 80% of sports bets are player props (Will Patrick Mahomes throw over 2.5 touchdowns?), not game outcomes (Chiefs vs. 49ers spread). This insight shapes product development and marketing—bettors are engaged with individual players, not just teams.

**Feedback (Incorrect):** This statistic surprised many in the industry. Andrew shares it to explain why FanDuel focuses marketing on player-level content and why their app prominently features player prop bets.

---

### Question 5: Marketing Philosophy

How does Andrew describe the balance between data science and creativity in marketing?

A) 100% data-driven—creativity is irrelevant at scale
B) 50/50 split between data analysis and creative intuition
C) "Equal parts science and art... the art is still 33%" ✅
D) 90% creative, 10% data—gut instinct wins

**Feedback (Correct):** Correct! Andrew acknowledges data is essential ("you can only measure what's happened in the past") but insists creativity remains critical—roughly one-third of marketing success. Data tells you what worked; creativity imagines what could work. FanDuel uses both to outperform competitors.

**Feedback (Incorrect):** Andrew pushes back on the assumption that a $1 billion budget means pure data optimization. He values creativity even at scale. What specific percentage does he assign to "the art" of marketing?

---

## Element 4: Details Accordion - Executive Bio

```
Title: About Andrew Sneyd

Andrew Sneyd serves as Chief Marketing Officer at FanDuel, where he leads all marketing strategy, brand development, customer acquisition, and retention for North America's largest sports betting and gaming operator.

**Career Highlights:**
- Leads $1B+ annual marketing investment
- Oversaw FanDuel's rise to #1 market position (~45% share)
- Created iconic "Kick of Destiny" Super Bowl campaign
- Built marketing operation across 20+ legalized states

**Key Achievements:**
- Brand awareness growth: 40% → 50% in one Super Bowl weekend
- 13 million customer sign-ups during single campaign
- All major media investments pay back within 18 months
- Pioneered "Trust, Easy, Fun" brand positioning

**Industry Context:**
FanDuel is owned by Flutter Entertainment, an Irish gambling conglomerate also operating PokerStars, Paddy Power, Betfair, and Sportsbet Australia. Under Andrew's leadership, FanDuel has become the dominant brand in US sports betting, outpacing DraftKings, BetMGM, and Caesars.
```

---

## Element 5: AI Chat Widget Configuration

**Widget Title:** Betting Marketing Q&A
**Persona:** Expert in sports betting marketing, brand strategy, and customer acquisition

**Sample Prompts:**
- "How did FanDuel's 'Trust, Easy, Fun' positioning differentiate from DraftKings?"
- "What made the Kick of Destiny campaign successful?"
- "How does FanDuel measure marketing ROI across 1 billion emails per year?"
- "What's the typical CAC and payback period for sports betting customers?"
- "Why are player prop bets (80% of bets) different from game outcome bets?"

**Context for AI:**
Students have watched the Andrew Sneyd interview and are learning about sponsorship ROI (WLO 3.1, 3.3) and sports betting as a growth driver (WLO 3.2). Help them understand betting marketing economics, brand building in regulated industries, and customer acquisition at scale. Reference specific examples from the interview (Kick of Destiny, 1 billion emails, Ohio launch).

---

## Module Complete - Transition to Module 2

**Completion Text:**
You've heard directly from the CMO of America's largest sports betting brand. Andrew's insights on brand building, customer acquisition economics, and the balance between data and creativity will inform your work throughout this week.

**Key Takeaways from Andrew Sneyd:**
- "Trust, Easy, Fun" provides a clear decision-making framework for all marketing
- Taking calculated creative risks ("the risk was in no one paying attention") can deliver outsized returns
- At scale ($1B budget), every channel must pay back within 18 months
- 80% of bets are player props—understanding customer behavior drives product and marketing strategy

**Up Next:** Module 2 - The Sponsorship Playbook
Now you'll build the frameworks to analyze sponsorship economics like Andrew and his team do—calculating CPM, designing activation budgets, and modeling ROI.

---

**Prepared for:** Week 3, Business of Sports Marketing
**Last Updated:** 2025-12-05
**Status:** Ready for implementation
